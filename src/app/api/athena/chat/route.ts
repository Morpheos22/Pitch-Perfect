...
export async function POST(request: NextRequest) {
  ...
  const quotaTier: "anon" | "auth" = userId ? "auth" : "anon";
  const identifier = userId ? userId : `${getClientIp(request)}:${await getDeviceFingerprint(request)}`;
  const quota = checkAthenaQuota(quotaTier, identifier);
  ...
  catch (error) { console.error("[Athena Chat] Error:", error); return NextResponse.json({ error: "Failed to get response from Athena" }, { status: 500 }); }
