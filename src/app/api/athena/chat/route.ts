...
import { getAthenaGreeting } from "@/lib/athena/response-library";
...
export async function POST(request: NextRequest) {
  const { userId } = await auth(); let body: unknown;
  ...
  const runtimeTier = tier === "deep" || tier === "diligence" ? tier : "conversational";
  const options = { reasoning: reasoning === true, tier: runtimeTier as "conversational" | "diligence" | "deep" };
  const quotaTier: "anon" | "auth" = userId ? "auth" : "anon"; const identifier = userId ? userId : `${getClientIp(request)}:${await getDeviceFingerprint(request)}`; const quota = checkAthenaQuota(quotaTier, identifier);
  ...
  try { let internalUserId: string | undefined; if (userId) internalUserId = (await prisma.user.findUnique({ where: { clerkId: userId }, select: { id: true } }))?.id; const ctx = userId ? { userId, internalUserId, firstName: (context as { firstName?: string } | null)?.firstName, currentModule: (context as { currentModule?: string } | null)?.currentModule, currentPage: (context as { currentPage?: string } | null)?.currentPage, plan: (context as { plan?: string } | null)?.plan } : undefined; const response = await askAthenaWithTools(message, ctx, (history as AthenaMessage[]) ?? [], options); return NextResponse.json({ response, timestamp: new Date().toISOString(), tier: quotaTier, remaining: quota.remaining, resetAt: quota.resetAt, nudgeSignIn: quotaTier === "anon" && quota.remaining <= ATHENA_QUOTA.ANON.limit - 2 }, { status: 200, headers: quotaHeaders({ tier: quotaTier, remaining: quota.remaining, resetAt: quota.resetAt }) }); }
