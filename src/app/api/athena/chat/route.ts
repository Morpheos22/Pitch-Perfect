...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = z.object({ message: z.string().min(1).max(2000), history: historySchema.optional().default([]), context: z.unknown().optional(), image: imageSchema.optional() }).strict().safeParse(body);
    if (!parsed.success) return NextResponse.json({ error: "Invalid request", details: parsed.error.flatten().fieldErrors }, { status: 400 });
    const { message, history, context, image } = parsed.data;
    ...
    const ctx = userId ? { userId, internalUserId, firstName: typedContext.firstName, currentModule: typedContext.currentModule, currentPage: typedContext.currentPage, plan: typedContext.plan } : undefined;
    const response = await askAthenaWithTools(message, ctx, history, image);
